...
def get_opts(Train=True):
    ...
    #                   x : 对应yolov8_x
    #############################################################################################    
    opt.net = 'yolov8'     # [centernet, faster_rcnn, retinanet, ssd, yolov3, yolov4, yolov5, yolov8, yolov8, yolox]
    opt.phi             = 's' # # yolov8的版本。
    opt.model_path      = 'model_data/weight/yolov8_%s.pth'%(opt.phi) #coco
    ...
    # #------------------------------------------------------#
    # opt.anchors_path    = 'det_model/yolov8/yolo_anchors.txt'
    ...
    #----------------------------------------------------#
    opt.UnFreeze_Epoch  = 300 #100
    opt.Unfreeze_batch_size = int(16/2)
    ...
    if Train:
        ...
        from det_model.yolov8.yolo import YOLO
        opt.Model_Pred = YOLO 
    return opt
