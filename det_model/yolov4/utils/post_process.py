...
class torch_post_process:
    ...
    def decode_box(self, inputs, input_shape=[608, 608], num_classes = 11, anchors = None,
                anchors_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]
                ):
        outputs = []
        inputs = [torch.from_numpy(o) for o in inputs]
        for i, input in enumerate(inputs):
            ...
            #-------------------------------------------------#
            scaled_anchors = [(anchor_width / stride_w, anchor_height / stride_h) for anchor_width, anchor_height in anchors[anchors_mask[i]]]
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4, official = False):
            #----------------------------------------------------------#
                    ...
                    if official:
                        ...
                        while detections_class.size(0):
                            ...
                            if len(detections_class) == 1:
                                break
                            ious = self.bbox_iou(max_detections[-1][0], detections_class[1:])
...
class np_post_process:
    ...
    def decode_box(self, inputs, input_shape=[608, 608], num_classes = 11, anchors = None,
        ...
        for i, input in enumerate(inputs):
            ...
            #-----------------------------------------------#
            bbox_attrs = num_classes + 5
            prediction = input.reshape(batch_size, len(anchors_mask[i]),
                                    bbox_attrs, input_height, input_width).transpose(0, 1, 3, 4, 2)
            ...
            x = sigmoid_v(prediction[..., 0])  
            y = sigmoid_v(prediction[..., 1])
            #-----------------------------------------------#
            #   先验框的宽高调整参数
            #-----------------------------------------------#
            w = prediction[..., 2]
            ...
            #   获得置信度，是否有物体
            #-----------------------------------------------#
            conf        = sigmoid_v(prediction[..., 4])
            #-----------------------------------------------#
            #   种类置信度
            #-----------------------------------------------#
            pred_cls    = sigmoid_v(prediction[..., 5:])      
            #----------------------------------------------------------#
            #   生成网格，先验框中心，网格左上角 
            #   batch_size,3,13,13
            #----------------------------------------------------------#    
            grid_x = np.tile(
                        np.tile(
                            np.linspace(0, input_width - 1, input_width),(1,input_height)
                        ).reshape(input_height, input_height),
                        (batch_size * len(anchors_mask[i]),1,1))
            ...
            grid_y = np.tile(
                        np.tile(
                            np.linspace(0, input_width - 1, input_width),(1,input_height)
                        ).reshape(input_height, input_height).transpose(),
                        (batch_size * len(anchors_mask[i]),1,1))        
            ...
            #   batch_size,3,13,13
            #----------------------------------------------------------#
            anchor_w = np.array([[e[0]] for e in scaled_anchors], dtype=np.float)
            anchor_h = np.array([[e[1]] for e in scaled_anchors], dtype=np.float)
        
            anchor_w = np.tile(
                np.tile(anchor_w, (batch_size, 1)),
                (input_height * input_width)).reshape(w.shape)

            anchor_h = np.tile(
                np.tile(anchor_h, (batch_size, 1)),
                (input_height * input_width)).reshape(h.shape)    
            #----------------------------------------------------------#
            #   利用预测结果对先验框进行调整
            #   首先调整先验框的中心，从先验框中心向右下角偏移
            #   再调整先验框的宽高。
            #----------------------------------------------------------#
            pred_boxes          = np.empty(prediction[..., :4].shape)
            pred_boxes[..., 0]  = x.data + grid_x
            pred_boxes[..., 1]  = y.data + grid_y
            ...
            #   将输出结果归一化成小数的形式
            #----------------------------------------------------------#
            _scale = np.array([input_width, input_height, input_width, input_height], dtype=np.float)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4, official = True):
            ...
            #   prediction  [batch_size, num_anchors, 85]
            #----------------------------------------------------------#
            box_corner          = np.empty(prediction.shape)
            box_corner[:, :, 0] = prediction[:, :, 0] - prediction[:, :, 2] / 2
            box_corner[:, :, 1] = prediction[:, :, 1] - prediction[:, :, 3] / 2
            box_corner[:, :, 2] = prediction[:, :, 0] + prediction[:, :, 2] / 2
            box_corner[:, :, 3] = prediction[:, :, 1] + prediction[:, :, 3] / 2
            ...
            for i, image_pred in enumerate(prediction):
                ...
                #   class_pred  [num_anchors, 1]    种类
                #----------------------------------------------------------#
                class_conf = np.amax(image_pred[:, 5:5 + num_classes], 1, keepdims=True)
