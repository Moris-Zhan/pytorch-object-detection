...
def get_opts(Train=True):
    ...
    #                   x : 对应yolov7_x
    #############################################################################################    
    opt.net = 'yolov7'     # [centernet, faster_rcnn, retinanet, ssd, yolov3, yolov4, yolov5, yolov7, yolox]
    opt.phi             = 'x' # # YoloV7的版本。
    opt.model_path      = 'model_data/weight/yolov7_%s.pth'%(opt.phi) #coco
    ...
    #------------------------------------------------------#
    opt.anchors_path    = 'det_model/yolov7/yolo_anchors.txt'
    ...
    if Train:
        ...
        from det_model.yolov7.yolo import YOLO
        opt.Model_Pred = YOLO 
    return opt
