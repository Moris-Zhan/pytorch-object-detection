...
def get_opts(Train=True):
    ...
    #############################################################################################    
    opt.net = 'rtmdet'     # [centernet, faster_rcnn, retinanet, ssd, yolov3, yolov4, yolov5, yolox]
    opt.phi             = 'l' # # rtmdet的版本。s、m、l、x
    opt.model_path      = "" #coco
    ...
    #------------------------------------------------------#
    opt.anchors_path    = 'det_model/rtmdet/yolo_anchors.txt'
    ...
    opt.Unfreeze_batch_size = int(8/1)
    ...
    if Train:
        ...
        from det_model.rtmdet.yolo import YOLO
        opt.Model_Pred = YOLO  
    return opt
