...
def get_opts(Train=True):
    ...
    #############################################################################################    
    opt.net = 'yolov5'     # [centernet, faster_rcnn, retinanet, ssd, yolov3, yolov4, yolov5, yolox]
    opt.phi             = 'm' # # YoloV5的版本。s、m、l、x
    opt.model_path      = 'model_data/weight/yolov5_%s.pth'%(opt.phi) #coco
    ...
    #------------------------------------------------------#
    opt.anchors_path    = 'det_model/yolov5/yolo_anchors.txt'
    ...
    if Train:
        ...
        from det_model.yolov5.yolo import YOLO
        opt.Model_Pred = YOLO  
    return opt
