...
# ----------------------------------------------
class AsiaTrafficDataset(Dataset):
    def __init__(self, img_size=416, batch_size = 2, is_training = True):
        # year = "2012"
        # mode = "trainval"
        root_path = "D:\\WorkSpace\\JupyterWorkSpace\\DataSet\\Object-Detection\\Asian-Traffic\\Stage1\\ivslab_train"
        self.image_path = os.path.join(root_path, "JPEGImages")  
        self.data_path = os.path.join(root_path, "Annotations")  
        ...
        self.img_files = glob(os.path.join(root_path, "JPEGImages\\All\\*\\*.jpg"))
        self.ann_file = glob(os.path.join(root_path, "Annotations\\All\\*\\*.xml"))
        # ---------
        #  Parse Json to Label
    ...
    def __getitem__(self, index):
        ...
        #  Image
        # ---------
        img_path = os.path.join(self.image_path, "{}.jpg".format(_id))
        ...
        # Pascal VOC Bounding box :(x-top left, y-top left,x-bottom right, y-bottom right)
        # ---------
        image_xml_path = os.path.join(self.data_path, "{}.xml".format(_id))
    ...
    def videoTest(self):
        out_path = "videoTest/AsiaTrafficDataset.mp4"       
        # input_image_folder = os.path.join(self.root_path, "VOC{}".format(self.year), "JPEGImages")
        input_image_folder = os.path.join(self.root_path, "JPEGImages")
        ...
        with tqdm(total = len(self.ids)) as pbar:
            for idx, id in enumerate(self.ids):
                ...
                image_xml_path = os.path.join(self.data_path, "{}.xml".format(id))
                annot = ET.parse(image_xml_path)
                objects = []
                for obj in annot.findall('object'):
                    xmin, xmax, ymin, ymax = [int(obj.find('bndbox').find(tag).text) - 1 for tag in
