...
def convert_annotation(VOCdevkit_path, classes,year, image_id, list_file):
...
def get_annotation(data_root):
    ...
    VOCdevkit_sets  = [('2007', 'train'), ('2007', 'val'), ('2012', 'train'), ('2012', 'val')]
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        for year, image_set in VOCdevkit_sets:
            ...
            dir_ = '%s/VOC%s/%s'%(VOCdevkit_path, year, "Detection")
            if not os.path.exists(dir_): os.makedirs(dir_)
            list_file = open('%s/VOC%s/%s/%s.txt'%(VOCdevkit_path, year, "Detection", image_set), 'w', encoding='utf-8')
            for image_id in tqdm(image_ids):
                list_file.write('%s/VOC%s/JPEGImages/%s.jpg'%(os.path.abspath(VOCdevkit_path), year, image_id))

                convert_annotation(VOCdevkit_path, classes, year, image_id, list_file)
                list_file.write('\n')
            list_file.close()
    ...
    for image_set in ["train", "val"]:
        ...
        for year in [2007, 2012]:          
            txts.append(os.path.join(VOCdevkit_path,"VOC%s"%(year), "Detection" ,"%s.txt"%image_set))
